...
def plot_vals(dfs, figname): 
 for i in range(dfs[0].shape[1]):
     ...
     for df in dfs:
         ...
         # ax.tight_layout()
     fig.suptitle(
                  f"Figure {i+3}: {str.title(df.columns[i]).replace("_"," ")} Trends Across London, Abuja and Bad Honnef",x=0,ha="left")
...
def plot_locs(dfs, figname): 
 for i in range(len(dfs)):
     ...
     ax.set_xlabel(f"{df.columns[0]}") 
     ax.legend(df.columns[1:]) 
     ax.set_title(f"Figure {i+1}: Weather parameters in {str(websites["Location"][i]).replace("_"," ")}",loc="left")
